...
party_change_and_econ_df <- read.csv("./data_processed/party_change_and_econ_df_v1.csv")
...
if(OUTPUT) {pdf(file = paste0("./output/model_evals", myFormulaName, ".pdf"))}
...
for (i in 1:nrow(data_df)) {
  ...
  if (i %% 100 == 0 & !OUTPUT) {print(paste0("Iteration: ", i, " out of ", nrow(data_df)))}
...
ggplot(data = roc_df, aes(x = logit_score, group = label, fill = label), alpha = 0.5) + 
  ...
  geom_text(aes(x=logit_ksplot$`KS Cutoff`+0.05, label="Logit Optimal Cutoff", y=230), colour="black") +
