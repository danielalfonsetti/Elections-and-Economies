...
# Model: Party Change within the next 4 years = Participation Rate + Unemployment Rate + GDP % Change

myFormulaName <- "model_1_4year_from_participation_unemployment_gdp_pchange"
...
for (i in 1:nrow(data_df)) {
  ...
  fitted_rf <- randomForest(
    formula = change_within_4years ~ participation_rate + unemployment_rate + gdp_pchange,
  ...
  roc_df[i,]$label <- cv_test_set$change_within_4years
