...
party_change_df <- party_change_df %>% 
  group_by(state, change_counter)  %>%
  mutate(change_within_4years = if (!is.na(years_till_change) && years_till_change <= 4) TRUE else FALSE) %>%
  mutate(change_within_4years = if (is.na(years_till_change)) NA else change_within_4years) %>%
  mutate(change_within_4years = ifelse(n() - row_number() >= 4, FALSE, change_within_4years))


party_change_df <- party_change_df %>% 
  mutate(change_next_year = if (!is.na(years_till_change) && years_till_change == 1) TRUE else FALSE) %>%
  mutate(change_next_year = if (is.na(years_till_change)) NA else change_next_year) %>%
...
# Save processed data 
x <- party_change_and_econ_df %>% filter(state == "Maine")
View(x)
...
for (cur_state in state.name) {
  ...
  state_df <- state_df %>% filter(year > 1977)
  p <- ggplot(state_df, aes(x = year, y = years_since_other_party, 
                            color = gdp_pchange, size = state_df$unemployment_rate, 
                            group = interaction(state, change_counter))) +
    geom_line(size = 1, color = "black") + 
    geom_point() + 
    ylim(c(-max(abs(state_df$years_since_other_party)), max(abs(state_df$years_since_other_party)))) + 
